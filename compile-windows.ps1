# Windows compile script for KrankyBearThreatInvaders / threatinvaders-windows (Windows build + optional packaging)
...
Write-Host "KrankyBear ThreatInvaders - Windows Compile Script" -ForegroundColor Cyan
...
Remove-Item -Path (Join-Path bin 'threatinvaders') -Force -ErrorAction SilentlyContinue

# Remove ALL syso files before generating new ones with go-winres
# This ensures we don't use stale/cached icons (e.g., old icon files)
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Current icon reference: ../Resources/Images/KrankyBearUSArmyCombatHelmetWoodland.png" -ForegroundColor Yellow
    Write-Host ""
} else {
...
if ($All -or $Windows -or (-not $Linux -and -not $Current)) {
    ...
    go build -ldflags="$ldflags" -trimpath -o bin/threatinvaders-windows.exe
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($Package) {
            ...
            try {
                Copy-Item -Path (Join-Path $PSScriptRoot "bin/threatinvaders-windows.exe") -Destination (Join-Path $PSScriptRoot "threatinvaders-windows.exe") -Force
            } catch {
            ...
            if (Test-Path $InnoPath) {
                & "$InnoPath" "Inno/KrankyBearThreatInvaders.iss"
                if ($LASTEXITCODE -eq 0) {
...
if ($All -or $Linux) {
    if ($env:OS -eq 'Windows_NT') {
        ...
        $env:GOOS = "linux"
        $env:GOARCH = "amd64"
        go build -ldflags="-s -w" -trimpath -o bin/threatinvaders-linux
        if ($LASTEXITCODE -eq 0) {
...
if ($Current) {
    ...
    go build -ldflags="-s -w" -trimpath -o bin/threatinvaders-windows
    if ($LASTEXITCODE -eq 0) {
...
if ($buildFailed) {
    ...
    Get-ChildItem -Path bin -Filter "*threatinvaders*" | Format-Table Name, Length -AutoSize
    ...
    if ($All -or $Windows -or (-not $Linux -and -not $Current)) {
        ...
        Write-Host "  go-winres extract bin\threatinvaders-windows.exe --dir extracted_resources" -ForegroundColor Cyan
    }
}
