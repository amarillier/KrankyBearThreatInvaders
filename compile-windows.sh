...
echo "KrankyBear ThreatInvaders - Windows Compile (bash)"
echo "================================================"
echo
...
rm -f bin/threatinvaders-windows.exe || true
...
if ! go build -ldflags="$LDFLAGS" -trimpath -o bin/threatinvaders-windows.exe
then
...
then
  ...
  cp -f bin/threatinvaders-windows.exe ./KrankyBearThreatInvaders.exe
  ISCC_DEFAULT="/c/Program Files (x86)/Inno Setup 6/ISCC.exe"
  if [[ -x "$ISCC_DEFAULT" || -f "$ISCC_DEFAULT" ]]
  then
    "${ISCC_DEFAULT}" ./Inno/KrankyBearThreatInvaders.iss || {
